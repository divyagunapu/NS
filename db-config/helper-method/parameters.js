...
module.exports = ( schema, payLoad) => {
       try {
        ...
        schema.Schema.forEach((param) => {
            param.alias = param.alias ? param.alias.trim() : "";
            param.column = param.column ? param.column.trim() : "";

            if (payLoad[param.column] !== undefined) {
                parameters.push('?'); // Use placeholder for parameter
            } else {
                parameters.push('NULL'); // Handle missing values if needed
            }
        });
        ...
        return {
            parameters: null,
            error: { "ERROR": "ERROR_WHILE_PREPARING_PARAMETERS", "MESSAGE": error.message }
        };
    }
}
